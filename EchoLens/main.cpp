    ...
    float margin)
{
    ...
    while (std::getline(iss, rawline)) {
        ...
        while (!line.empty()) {
            ...
            HPDF_UINT cut = HPDF_Font_MeasureText(
                font,
                data,
                len,
                maxWidth,
            ...
            std::string segment = line.substr(0, cut);
...
void createRefinedPDF(const std::string& filename, const std::string& readableText) {
    ...
    std::cout << "[PDF] Created: " << filename << "\n";
}

string refinedToReadableText(const json& j) {
    ...
    auto writeValue = [&](const std::string& label, const std::string& value) {
        if (!value.empty())
            out << label << ": " << value << "\n";
        };

    auto writeArray = [&](const std::string& label, const json& arr) {
...
string makePdfName(const std::string& name) {
    string clean;
...
string askUserForSaveLocation(const std::string& suggestedName) {
