//// try_streaming_filtered.cpp
////
//// Final patched MSVC-friendly version (Live streaming + Keyword pre-filter + Lexbor kept):
//// - Streams results per URL (prints immediately and flushes).
//// - Performs a quick pre-filter for contact-related keywords (case-insensitive) to skip heavy parsing when unlikely.
//// - Tries Lexbor parsing inside std::async with hard timeout (fallback to simple stripper).
//// - Keeps cURL timeouts and download size guard.
//// - maxProcess = 3 by default for quick debug runs.
//// - TODO section at top for easy adjustments.
////
//// IMPORTANT:
...
//static const size_t MAX_DOWNLOAD_BYTES = 8 * 1024 * 1024; // 5 MB maximum to download per page
...
//static const chrono::seconds LEXBOR_TIMEOUT(20);   // wait up to 5 seconds for Lexbor parsing
//// ------------------ end Configuration ------------------
//
